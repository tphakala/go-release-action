...
RELEASE_ASSET_NAME=${BINARY_NAME}-${RELEASE_TAG}_${INPUT_GOOS}_${INPUT_GOARCH}
if [ ! -z "${INPUT_GOAMD64}" ]; then
  RELEASE_ASSET_NAME=${BINARY_NAME}-${RELEASE_TAG}_${INPUT_GOOS}_${INPUT_GOARCH}_${INPUT_GOAMD64}
fi
if [ ! -z "${INPUT_GOARM}" ] && [[ "${INPUT_GOARCH}" =~ arm ]]; then
  RELEASE_ASSET_NAME=${BINARY_NAME}-${RELEASE_TAG}_${INPUT_GOOS}_${INPUT_GOARCH}v${INPUT_GOARM}
fi
